...
def test_system():
    ...
    dist_path = os.path.join(current_dir, "frontend_smart_presence", "frontend_smart_presence", "dist")
    if os.path.exists(dist_path) and os.path.isdir(dist_path):
        print(f"[SUCCESS] Frontend Build (dist) found at: {dist_path}")
    else:
        print(f"[FAILURE] Frontend Build (dist) NOT FOUND at {dist_path}")
        print("          Try running 'npm run build' in frontend folder.")
